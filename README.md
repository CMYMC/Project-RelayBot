...
직접 통신을 통해 네트워크르라 형성하고 통신 품질 저하가 감지되면 스스로 최적의 위치로 이동하여 연결을 복구 및 확장하는 지능형 로봇 시스템
...
- **자율 복구 시연 성공:** 호스트와 단말기 사이의 통신이 끊어지는 음영 지역 발생 시, Relay-bot이 자동으로 이동하여 TQ 값을 회복하고 영상 연결을 복구함
- **인프라 종속성 제거:** 외부 공유기나 인터넷 없이 로봇과 단말기들만으로 독립적인 통신망 구성 가능성을 입증.
- **경량화된 제어:** 고사양의 SLAM/Navigation 스택 없이 신호 세기만으로 목적을 달성하는 알고리즘 구현.
